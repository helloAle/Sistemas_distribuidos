...
def add_item_to_database(item_name, item_quantity):
...
def remove_item_from_database(item_name):
    # Conectar ao banco de dados
    conn = sqlite3.connect('nadespensa.db')
    c = conn.cursor()
...
def update_item_in_database(item_name, item_quantity):
    # Conectar ao banco de dados
    conn = sqlite3.connect('nadespensa.db')
    c = conn.cursor()
...
def handle_message(conn, type, data):
    if type == 'MSG':
        ...
        add_item_to_database(item_name, item_quantity)
        # Enviar uma mensagem de resposta
        ...
        item_name = data
        remove_item_from_database(item_name)
        # Enviar uma mensagem de resposta
        ...
        item_name, item_quantity = data.split('|')
        update_item_in_database(item_name, item_quantity)
        # Enviar uma mensagem de resposta
        ...
        item_name = data
        item = get_item_from_database(item_name)
        if item:
